...
//		System.out.println(s);
	...
	public static void  test5(int year,int month,int day) {//2010  10 10
		 ...
		 //判断参数年月日是否合理
		  if(year<2000) {
			  System.out.println(year+"年"+month+"月"+day+"日不在2000 1 1日之后 日期不合理"); return;
		  }
		  int days=0;//定义变量记录天数
		  for (int yearN = 2000; yearN < year; yearN++) {//定义for循环跑完所有的年
			  //获取yearN年的天数
			  boolean b=isRunNian(yearN);//判断当前年是不是闰年
			  if(b==true) {days+=366; }else { days+=365; }
		  }
		  for (int monthN = 1; monthN < month; monthN++) { //把year年的前month-1个整月跑完
			   days+=getDays(year, monthN);
		  }
		  days+=day; //把month月已过的天数加上
		  int ys=days%5; //获取余数
		  if(ys==0||ys==4) {
			  System.out.println(year+"年"+month+"月"+day+"日在晒网！");
		  }else {
			  System.out.println(year+"年"+month+"月"+day+"日在打渔！");
		  }
	}
	//写一个方法判断是否为闰年
